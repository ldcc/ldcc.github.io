...
$$
\sum_{i} c_i \vm{v}^{(i)},
\qquad \vm{v}^{(i)} \in \{\vm{v}^{(1)}, \dots, \vm{v}^{(n)}\}
$$
...
$$
\norm{\vm{x}}_p = (\sum_i |x_i|^p)^{\frac{1}{p}},
...
$$
\norm{\vm{A}}_F = (\sum_{i,j} (A_{i,j})^2)^{\frac{1}{2}}
$$
