...
技术有多牛都是次要的，能在面试官前吹出多少能唬到人的 B 才是最重要的。
...
区块链由多个分布式的节点共同维护与运行，每一个单独的节点同时可以看作是一个「数据服务器」。
以 Ethereum 为例：一段智能合约代码在客户端本地完成编译后通过 sockets 发送到服务器，一个 eth 
节点中随时都运行着一个 EVM 以处理区块链中的交易，EVM 接收到该份代码后再交给该智能合约的解释器执行，
最后把执行结果返回客户端，一次 RPC 通信就完成了。
