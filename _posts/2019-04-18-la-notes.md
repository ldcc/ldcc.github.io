...
Norms is a function that uses to measure the magnitude of a vector. Formally, the $$L^p$$ norms given by:
...
When $$p=1$$, then $$L^1$$ norm may be simplified to:

$$
\norm{\t{x}}_1 = \sum_i |x_i|
$$

If you like, $$L^2$$ norm could be more intuitive to interpret by [Pythagorean Theorem][Pythagorean]:

$$\vec{o}^{\,t}$$

travel from the origin.
