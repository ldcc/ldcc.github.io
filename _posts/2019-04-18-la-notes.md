...
对于 $$5 \times 0 = 0$$ 这条公式， 请问能够对其做逆运算而还原出原式中的除数吗？显然这个世界并不存在 $$0 \div 0 = 5$$ 这种数学。
在做这个乘法的逆运算时，这条公式缺少了能够从 $$0$$ 到 $$5$$ 的关键因子。

矩阵的行列式为 $$0$$ 时该矩阵不可逆，这种说法只是将 $$0$$ 不能当被除数抽象成一种更为普遍广泛的说法而已。

同样地对于一个 $$n \times n$$ 的方阵 $$\t{A}$$ 而言，如果：

- $$\t{A}$$ 是线性无关的，但 $$\norm{\t{A}} = 0$$，那么 $$\t{A}$$ 在做线性变换时就会把一组线性无关的向量变换成线性相关的，这个过程显然会丢失部分信息，不可逆。
- $$\t{A}$$ 是线性相关的，由于一对线性相关的向量所张成的面积是 $$0$$，相当于 $$\t{A}$$ 天生就缺少一个纬度，不可逆。
...
Sometimes we may also wish to measure the size of a matrix, the most common way to do this is with the __Frobenius norm__:
