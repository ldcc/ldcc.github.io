...
说到 VCS，就不得不提一下 Git 使用的一种叫 Merkle DAG 的数据结构，DAG 也被实际应用到了 IPFS 以及区块链应用中。
...
- 创建一棵 Merkle Tree 的时间复杂度为 **O(n)**，这里指 chf 的执行次数。
- 生成的 Merkle Tree 的空间复杂度为 **O(log(n))**，这里指树的高度。
- 查找元素的时间复杂度为 **O(log(n))**，这里指比较的次数。
- 单纯判断两组数据(roothash)的等价关系则只需 **O(1)**。
