...
四则运算是我们平时使用最频繁的操作之一，由于它实在是太过普遍，我们很少去考虑去自己实现这一过程，由于新电脑终于有着落了，同时也是好久没有写博客了，那么先随便写点东西练练手。在这篇文章中，我将使用 lisp，并通过一些常规的递归手段来实现这几个基本过程。
...
我们的语法是在太简单，它的数据结构就如上面那张图的数据结构一样。因此我们这个计算器的算法，实际上就是在这棵树上的一种遍历操作。
